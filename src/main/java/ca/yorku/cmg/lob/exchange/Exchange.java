...
public boolean validateOrder(IOrder o) {
    ...
    // Get the trader’s position in the security mentioned in the order
    int pos = accounts.getTraderAccount(o.getTrader()).getPosition(o.getTicker());

    // Get the balance the trader has with the exchange
    long bal = accounts.getTraderAccount(o.getTrader()).getBalance();

    // Does ask trader have position in the security sufficient for a sell?
    if ((o instanceof Ask) && (pos < o.getQuantity())) {
        System.err.println("Order validation: seller with ID " + o.getTrader().getID() + 
            " not enough shares of " + o.getTicker() + ": has " + pos + " and tries to sell " + o.getQuantity());
        return false;
    }
    ...
    if (o instanceof Bid) {
        // Go to the asks half-book, see if there are matching asks and process them
        oOutcome = book.getAsks().processOrder(o, time);

        // If the quantity of the unfulfilled order is not zero
        if (oOutcome.getUnfulfilledOrder().getQuantity() > 0) {
            // Add the unfulfilled part to the bid half-book
            book.getBids().addOrder(oOutcome.getUnfulfilledOrder());
        }
    } else { // order is an ask
        // Go to the bids half-book and see if there are matching bids and process them
        oOutcome = book.getBids().processOrder(o, time);

        // If the quantity of the unfulfilled order is not zero
        if (oOutcome.getUnfulfilledOrder().getQuantity() > 0) {
            // Add the unfulfilled part to the ask half-book
    ...
    for (ITrade t : oOutcome.getResultingTrades()) {
        ...
        accounts.getTraderAccount(t.getBuyer()).deductBalance(buyerFee);
        accounts.getTraderAccount(t.getBuyer()).deductBalance(t.getTotalPrice());
        ...
        accounts.getTraderAccount(t.getSeller()).deductBalance(sellerFee);
        accounts.getTraderAccount(t.getSeller()).addBalance(t.getTotalPrice());
